...
router.post("/", async (req, res) => {
    try {
        ...
        if (!content) {
            return res.status(500).json({ message: "AI response invalid or missing." });
        }

        // Extract JSON from content
        const jsonMatch = content.match(/\{[\s\S]*\}/);
        if (!jsonMatch) {
            return res.status(500).json({ message: "Failed to extract JSON from AI response." });
        }

        const evaluation = JSON.parse(jsonMatch[0]);

        // Optionally save evaluation to disk
        const filePath = path.join(__dirname, "../data/interviews.json");
        ...
        // Send the evaluation back to frontend
        res.json(evaluation);

    } catch (error) {
        console.error("❌ Error in /api/finaleval:", error.message);
